...
function initScrollSpy() {
    const targets = collectSpyTargets();
    if (targets.length === 0) {
        return;
    }
...
function initPhotoOverlay() {
    const photo = queryRequired("#profilePhoto");
    const overlay = queryRequired("#photoOverlay");
...
function initProjectModal() {
    const backdrop = queryRequired("#projectModalBackdrop");
    const content = queryRequired("#modalContent");
    const closeButton = queryRequired("#modalClose");
